...
FROM
	customer_purchases WHERE vendor_id >=8 AND vendor_id <=10;
...
SELECT
	...
	quantity * cost_to_customer_per_qty AS price
FROM
	customer_purchases WHERE vendor_id BETWEEN 8 AND 10;
...
FROM
	vendor AS v
INNER JOIN
	vendor_booth_assignments AS vb
ON
...
SELECT
	vendor_id,
	COUNT(vendor_id) as num_of_booth_assignments
FROM
...
GROUP BY 
    c.customer_id
HAVING
    ROUND(SUM(quantity*cost_to_customer_per_qty),2) > 2000
ORDER BY 
