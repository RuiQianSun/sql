...
SELECT
    ...
    transaction_time,
	COUNT(*) OVER (PARTITION BY customer_id, product_id) AS total_number_of_purchases
FROM customer_purchases;
...
ranked_sales AS (
    -- Step 2: Rank the market dates based on total sales
    SELECT
        market_date,
        total_sales,
...
FROM
	(SELECT DISTINCT
		v.vendor_name,
		p.product_name,
		vi.original_price * 5 AS selling_five
		FROM 
			vendor_inventory vi
		LEFT JOIN 
...
VALUES 
(101, 'Apple Pie', 'Small', 1, 'unit', CURRENT_TIMESTAMP);
...
SET current_quantity = (
    ...
    AND vi.market_date = (
        SELECT MAX(vi_inner.market_date)
        FROM vendor_inventory vi_inner
        WHERE vi_inner.product_id = vi.product_id
    )
)
WHERE product_id IN (
